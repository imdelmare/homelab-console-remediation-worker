...
# Codex is opt-in so the default image retains the smallest attack surface.
ARG INCLUDE_CODEX=false
RUN if [ "${INCLUDE_CODEX}" = "true" ]; then npm install --global "@openai/codex@${CODEX_VERSION}" && test "$(codex --version)" = "codex-cli ${CODEX_VERSION}"; fi
