...
class ClaudeEngine:
    """Closed Claude Code launch profile. Output is bounded and discarded."""
    ...
    def __init__(self, project_dir: Path, timeout_seconds: int, profile_config: Path | None = None, prompt: str = "", env: dict[str, str] | None = None) -> None:
        ...
        self._env["ANTHROPIC_API_KEY"] = source.get("ANTHROPIC_API_KEY", "")
        self._process: asyncio.subprocess.Process | None = None

    def argv(self) -> list[str]:
        return ["claude", "-p", self._prompt, "--dangerously-skip-permissions"]
