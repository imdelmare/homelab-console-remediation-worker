...
2. Create `secrets/mcp-token` containing the dedicated MCP bearer token and
   `secrets/auth.json` containing the OpenCode provider authentication
   exported from `~/.local/share/opencode/auth.json` (or equivalent for the active engine). Never commit either file.
...
5. Copy the engine profile from `profiles/` to the config path. Do not add other MCP servers or allow a user/project config to override this closed profile.
